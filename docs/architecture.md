...
- **Handlers**: one file per domain in `internal/handler/` — workspace, run, variables, teams, state, audit, etc. Handlers call services, services call the repository layer.
...
- Picks up `run` jobs enqueued by the server
- Fetches workspace config, variables, and previous state
...
- **Styling**: Tailwind CSS 4 with shadcn-style component primitives

## Data Stores

### Postgres

Primary data store and job queue. All application data lives here — workspaces, runs, variables, teams, users, audit logs. River uses Postgres advisory locks and a jobs table for reliable queue semantics.
